...
/* Sort grid items alphabetically by 'data-creator-title' */
const sortGrid = () => {
	const sortedItems = gridItems.sort((a, b) => {
		const nameA = a.getAttribute("data-creator-title").toLowerCase();
		const nameB = b.getAttribute("data-creator-title").toLowerCase();
...
const filterGrid = (searchValue) => {
	...
	for (const item of gridItems) {
		...
		const creatorTitle = item.getAttribute("data-creator-title").toLowerCase();

		item.style.display =
			creatorName.includes(lowerCaseSearch) ||
			creatorTitle.includes(lowerCaseSearch)
				? ""
				: "none";
	}
};
