...
class CreatorTooltip {
	constructor(gridEl) {
		this.grid = gridEl;
		this.creators = this.grid.children;
		...
		this.rowAnimationDirections = {
			creatorTitle: { in: { yPercent: -100 }, out: { yPercent: -100 } }, // In and out to/from the top
		...
		this.hoverTarget = null; // Tracks the currently hovered `.creator`
	...
	initializeEvents() {
		...
		for (const creator of [...this.creators]) {
			creator.addEventListener("mouseenter", this.handleMouseEnter);
			creator.addEventListener("mouseleave", this.handleMouseLeave);
		}
	}
	...
	handleMouseMove = (e) => {
		...
		this.scaleDownTimeout = setTimeout(() => {
			if (!this.hoverTarget) {
				...
				this.updateTooltip(
					{ creatorTitle: "", creatorName: "", price: "" },
	...
	handleMouseEnter = (e) => {
		...
		const creatorTitle = this.hoverTarget.dataset.creatorTitle;
	...
	destroy() {
		...
		for (const creator of [...this.creators]) {
			creator.removeEventListener("mouseenter", this.handleMouseEnter);
			creator.removeEventListener("mouseleave", this.handleMouseLeave);
		}
	}
	...
	updateTextSlider(rowSelector, newValue, timeline, direction) {
		...
		if (direction === "in") {
			...
			const transitionOutDirection = {
				creatorTitle: { yPercent: 100 }, // Slide down for creatorTitle
...
const handlePageEvent = (type) => {
	...
	if (type === "load") {
		tooltip = new CreatorTooltip(document.querySelector("[data-grid]"));
